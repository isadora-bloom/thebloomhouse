# Bloom House — Consolidation Plan (Phased) · v2

**Date:** 2026-05-22 · v2 closes 12 gaps found in a critical pass of v1.
...
**1.3 — Shadow-compare each writer before flipping (gap #4).** Every migrated writer runs in shadow first: old path + new cascade path both write, a comparator logs any divergence (couple bound, touchpoint count, fields). Flip the writer to cascade-only when divergence is zero across a representative sample. This is the per-writer safety net — not just the end-of-phase check.
