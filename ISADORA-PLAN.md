...
- [x] **ContractHouse audit.** Done 17 Sep: `contracthouse/AUDIT-2026-09-17.md`. Ten broken things (raw placeholders in signed contracts, audit trail never writes, promised emails that don't exist, no Stripe in prod, no state checks on sign or pay, double-record and wrong-amount payments, milestone status never moves, reminders to partner 1 only, AI cost 3x, no tests). Still missing from v1: co-signer, certificate of completion, signed-copy email, reminder cadence UI, clause library. Production holds only test contracts.
