...
- T8.1d — tier-layering + tombstoning + snapshot reload. Sort the sweep into `signal_tier` layers (high mints precede low attaches); between layers re-load the couples snapshot (today taken once at sweep start) and tombstone every `candidate_matches` row consumed by a minted couple so the next layer's matcher excludes it.
