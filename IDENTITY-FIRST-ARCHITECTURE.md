...
- T8.1b ✅ **DONE** — wire the mint. The mint lives in `applyTierRouting`'s `below_threshold` branch (the shared routing primitive — not just `processSignal`), so the **Forwards Linker gets it too**: an identity-sufficient signal the matcher placed against no couple mints a `channel_scoped` couple via `lockAndMintCouple`; an identity-poor one stays a Fragment. `couples_minted` is now incremented; `'minted'` is a first-class `TierRoutingAction`/`LinkAction`. Cold-start venues now bootstrap their first couple from a live inbound. **The `signal_tier` sort moved to T8.1d** — sorting without the snapshot reload buys nothing (a low-tier signal still matches the stale pre-mint snapshot), and the email/phone double-mint race is already closed by the RPC's in-lock re-check, so the sort is a quality, not a correctness, change. It ships with the reload.
- T8.1c ✅ **DONE** — rebuilt `cross_channel_coalesce`. The stage now scores every cross-channel fragment-pair (within the 14-day window, bucketed by `identity_hint` as a cheap pre-filter) with the real structured matcher. `score > 90` (above the matcher's judge band) auto-promotes the pair into a `channel_scoped` couple — `promoteFragmentInto` mints the couple, sets both `fragments.promoted_to_couple_id`/`promoted_at`, and writes a `couple_merge_events` `fragment_promoted` row; a per-run `fragment_id → couple_id` map handles transitive promotion and flags pairs that bridge two already-coalesced couples for operator review. Pairs scoring 30–90 queue a `candidate_match` at the matcher's tier (the LLM judge for this Fragment band is deferred — `buildJudgeContext` is couple-shaped, not pair-shaped). Migration 360 fixes the `candidate_matches` CHECK gap — `route-by-tier.ts`'s medium/low branch writes `secondary_record_type='touchpoint'`, which mig 346's CHECK rejected, so `insertCandidateMatch` was silently swallowing every medium/low live match; the CHECK now allows `'touchpoint'`.
- T8.1d ✅ **DONE** — signal_tier layering for the sweep, re-cut after a verify pass. The original wording ("sort the sweep into signal_tier layers; between layers re-load the snapshot and tombstone candidate_matches") does not survive contact with the code: a streaming signal source cannot be globally sorted by `signal_tier` without buffering every signal (the `types.ts` header forbids it — ~100K+ per venue) or re-walking each adapter once per tier (6× the DB reads); and the matcher scores against `couples`, never against `candidate_matches`, so there is no candidate_matches snapshot to "tombstone so the next pass's matcher excludes it" — duplicate `candidate_matches` are already prevented by `uq_candidate_matches_pair` (T8.0a). Two cheap mechanisms in `stageTouchpointSweep` reach the same end (a later signal sees the couples an earlier signal minted): **(1) channel ordering** — adapters sweep high-signal channel first (`CHANNEL_SWEEP_RANK`: calendly/honeybook → gmail/sms → web → knot/weddingwire → instagram/review), so minting channels run before attaching channels; **(2) snapshot reload** — the couples snapshot is re-loaded every 500 signals (one cheap query) so a low-tier signal after a high-tier mint matches the fresh couple instead of orphaning.
- T8.1e ✅ **DONE** — checkpointing wired. `runTracer` + `getResumeFrom` already implemented stage-level resume (a prior `run_id` skips every `succeeded` stage), but nothing ever passed a `run_id` back, so a timed-out sweep redid the whole run. `tracer-runner.ts` now calls `findResumableRunId` before each venue run: it reads the venue's most recent `tracer_run_events.run_id` and resumes it unless that run already emitted a `validate`/`succeeded` (normal finish) or `anchor_discovery`/`skipped` (cold-start terminal). The drain's existing 8-minute in-progress guard means this only ever fires for a genuinely crashed run. Resume is stage-granular (a re-run of `touchpoint_sweep` is idempotent via the UNIQUE constraints + the RPC re-check); per-batch resume is a future optimisation. An explicit `opts.runId` still wins. (Folded in from the retired T8.0d.)
