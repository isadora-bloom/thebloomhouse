...
test.describe('§13 Phase 2 — Client Lifecycle + Availability', () => {
  ...
  test('076: wedding_timeline table is gone; ceremony_start lives on weddings', async () => {
    // (This RPC probe did nothing: it resolved to { data: null } either
    // way and `tbl` was never read. Left out rather than left in — the
    // assertion that matters is the column check below.)
