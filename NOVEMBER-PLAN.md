...
- [x] **DEPLOYED 2026-09-15:** master fast-forwarded to f63e5c51 (203 commits, waves 1 to 9 plus the security remediation) after `npm run preflight` 22/22. Migrations 291 to 413 on production, types regenerated, four secrets set via `scripts/deploy-secrets.mjs`, Calendly subscription recreated with a fresh token (the stored one was invalid, so Calendly polling had been dead). Production smoke-checked after the build (headers, redirect, webhooks and cron fail closed, demo API path closed, anonymous invite refused). Demo reseed applied on production (spine populated on all four demo venues, VERIFY PASS); 13 extra-row errors from W70 (a phantom table and two CHECK sets) are W72's class fix, then `--only-aux`. Still owed: **rotate the Supabase service-role key** (in public repo history, never rotated), Gmail reconnect, the reimport, Stripe when it is wired.
- [ ] **Deploy runbook for next time (`docs/DEPLOY.md`):** `npm run preflight` must pass; then the four
