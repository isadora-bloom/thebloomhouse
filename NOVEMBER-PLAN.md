...
- [x] **DEPLOYED 2026-09-15:** master fast-forwarded to f63e5c51 (203 commits, waves 1 to 9 plus the security remediation) after `npm run preflight` 22/22. Migrations 291 to 413 on production, types regenerated, four secrets set via `scripts/deploy-secrets.mjs`, Calendly subscription recreated with a fresh token (the stored one was invalid, so Calendly polling had been dead). Still owed after the deploy: `npx tsx scripts/demo-reseed.ts --apply --allow-prod` (demo spine is empty on production), Gmail reconnect, the reimport, Stripe when it is wired.
