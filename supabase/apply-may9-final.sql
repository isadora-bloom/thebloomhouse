-- ---------------------------------------------------------------------------
-- Combined apply: migrations 246 + 247 + identity backfill
-- ---------------------------------------------------------------------------
-- Renamed file (the older apply-pending-may8-deep.sql had cache issues).
-- Paste into https://supabase.com/dashboard/project/jsxxgwprxuqgcauzlxcb/sql/new
-- Idempotent. Safe to re-run.
-- Includes:
--   - Schema introspection before each FK reassign
--   - EXCEPTION WHEN unique_violation handlers that delete colliding
--     loser rows when a UPDATE would violate a unique constraint
--     (e.g. uq_engagement_events_fire_once)
