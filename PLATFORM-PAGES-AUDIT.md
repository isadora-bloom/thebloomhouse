...
| `/admin/imports/upload` | The same CRM import form the onboarding step uses, reachable on a Monday without the onboarding project | "Import a file" on `/agent/leads` and the button on `/admin/imports`; no sidebar entry by design (it is an action, not a destination) | **keep** — added 2026-09-14 by Wave 6 W42; linked from two kept pages, so not an orphan |
