...
| `/intel/marketing-roi/recommendations` | Reallocation recommendations, full status list | Nothing — `MarketingRecommendationsPanel.tsx` imported only by this page itself; `MarketingRoiDashboard.tsx` (the actual content of `/intel/marketing-roi`) links only back to `/intel/channels`, never forward to these three | **hide (scaffold)** |
