// Stream QQ step 2: refresh source_attribution rollup with NN's
// year-bucketing rewrite (post-180/181). Inlines refreshAttributionAllVenues
// from src/app/api/cron/route.ts so we can run it without an HTTP cron
// trigger and capture the resulting row distribution.
...
async function main() {
  ...
  // Pre-snapshot current source_attribution
  const { count: beforeCount } = await sb
    .from('source_attribution').select('id', { count: 'exact', head: true })
    .eq('venue_id', RIXEY_ID)
  console.log(`source_attribution rows for Rixey BEFORE: ${beforeCount}`)
