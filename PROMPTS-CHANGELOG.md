...
| --- | --- | --- | --- |
| askIntel (`src/lib/intel/canonical.ts`) | `ask-intel.tools.prompt.v1.0` | `ask-intel.tools.prompt.v1.1` | NOVEMBER-PLAN.md wave 2. The manifest is no longer fixed at five readers: `src/lib/intel/tool-sources/` is a plug-in registry and the prompt now lists each registered source's subjects in the "You have access to" line (`scopeSummaryFor`), so a question a source covers is not refused as out of scope. Rules 1 to 8 unchanged. `OUT_OF_SCOPE_SUBJECTS` is pruned by hand as sources land. With no sources registered the prompt text is byte-identical to v1.0. |
