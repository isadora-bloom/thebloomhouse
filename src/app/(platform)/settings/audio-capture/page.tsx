 ...
 *
 * T2-E Phase 2 (2026-05-01): provider-agnostic settings surface for
 * audio-capture. OMI Dev Kit 2 is the first provider; iPhone upload /
 * Otter / AssemblyAI / Deepgram fit into the same shape (each provider
 * gets its own card here as adapters land).
 *
 * What this page can do (OMI provider):
...
export default function AudioCaptureSettingsPage() {
  ...
  if (loading) {
    return <div className="text-sm text-sage-500 p-6">Loading audio-capture settings...</div>
  }
  ...
  return (
    <div className="max-w-3xl space-y-8">
      <header className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-serif text-sage-900">Audio Capture</h1>
          <p className="text-sm text-sage-600 mt-1">
            Capture tour conversations through OMI today (iPhone upload, Otter,
            AssemblyAI, and Deepgram coming as adapters land). {aiName} learns
            from what was said and transcripts attach to the matching tour
            automatically so you never have to write up a recap.
          </p>
        </div>
      </header>

      <section className="border border-sage-200 rounded-lg bg-sage-50/40 p-4 space-y-2">
        <h2 className="text-sm font-medium text-sage-800">Provider: OMI Dev Kit 2</h2>
        <p className="text-xs text-sage-600">
          The OMI wearable is the only provider currently shipping. Configure
          its webhook below. Per-provider settings will appear here as
