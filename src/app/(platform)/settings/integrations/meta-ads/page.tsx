 ...
 *
 * The only on-product face of the Meta Ads connector. Shows whether the
 * app credentials exist, whether this venue has connected, which ad
 * account the daily read points at, and when it last ran. Tokens never
 * come near this page: everything here is served by
 * /api/integrations/meta-ads/status, which names its columns and omits
 * the token ones.
 */
...
export default function MetaAdsIntegrationPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const resp = await fetch('/api/integrations/meta-ads/status')
      if (!resp.ok) {
        setState({
          configured: false,
          missing: [],
      ...
      setState(j)
      setAccountInput(j.connection?.ad_account_id ?? '')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const saveAccount = async () => {
    ...
    try {
      const resp = await fetch('/api/integrations/meta-ads/status', {
        ...
        body: JSON.stringify({ adAccountId: accountInput }),
      })
      if (!resp.ok) {
        const j = (await resp.json().catch(() => null)) as { error?: string } | null
        setSaveError(j?.error ?? 'That ad account could not be saved.')
        return
      }
      await load()
    } finally {
    ...
    <div className="space-y-6 p-6">
      <header>
        ...
        <p className="mt-1 max-w-2xl text-sm text-[var(--bh-muted)]">
          Connect your Facebook and Instagram ad account and Bloom reads
          the daily spend, impressions, clicks and enquiries for each
          campaign straight from Meta. Until you do, those numbers are
          whatever somebody typed in, and the reallocation page says so.
        </p>
      </header>
      ...
      {errFromCallback ? (
        <div className="flex items-start gap-2 rounded-md border border-rose-200 bg-rose-50 p-3 text-sm text-rose-900">
          ...
          Meta sent us back with: <code>{errFromCallback}</code>
        </div>
      ) : null}
      ...
      {!state.configured ? (
        <section className="rounded-2xl border border-amber-200 bg-amber-50/50 p-5 shadow-sm">
          ...
          <p className="mt-2 text-sm text-amber-900">
            Meta Ads needs three settings in place before any venue can
          ...
          <details className="mt-4 text-sm text-amber-900">
            ...
            <ol className="mt-3 list-decimal space-y-2 pl-5 text-xs">
              <li>
                Create an app in the Meta developer console and add the
                Marketing API product to it.
              </li>
              <li>
                Add this address as a valid redirect:{' '}
                <code>
              ...
              <li>
                Request the <code>ads_read</code> and{' '}
                <code>business_management</code> permissions. Read access is
                all this needs; nothing here can change a campaign.
              </li>
              <li>
                Set <code>META_ADS_APP_ID</code>,{' '}
                <code>META_ADS_APP_SECRET</code> and{' '}
                <code>META_ADS_OAUTH_REDIRECT_URI</code> in the hosting
          ...
          {state.connectorStatus === 'connected' ? (
            <div className="mt-3 space-y-3">
              ...
              {connection?.ad_account_id ? (
                <p className="text-sm">
                  Reading ad account{' '}
                  <span className="font-mono">{connection.ad_account_id}</span>
                ...
                <p className="text-sm text-amber-800">
                  Connected, but no ad account has been chosen yet, so
                  nothing is being read. Enter the account id below.
                </p>
              )}
            ...
            <div className="mt-3 space-y-3">
              <p className="text-sm text-[var(--bh-muted)]">
                Not connected. Meta will ask you to grant read access to
                your ad account. Bloom cannot change a campaign, set a
                budget or spend anything; it only reads what already
                happened.
              </p>
              <button
                type="button"
              ...
              >
                <PlugZap className="h-4 w-4" /> Connect Meta Ads
              </button>
            </div>
          )}
        </section>
      ...
      <section className="rounded-2xl border border-[var(--bh-line)] bg-white p-5 shadow-sm">
        ...
        <ul className="mt-3 space-y-2 text-sm">
          <li>
            <strong>Spend per campaign per day.</strong> The figure Meta
            has, on the day it happened, rather than a monthly total
            copied across by hand.
          </li>
          <li>
            <strong>Impressions, clicks and enquiries.</strong> The last of
            those comes from Meta&apos;s lead actions only. A video view is
            not somebody asking about your venue, and counting it as one
            would flatter every cost-per-enquiry number on the page.
          </li>
          <li>
            <strong>Corrections.</strong> Meta restates a day for a while
            after it closes. Each run rewrites the day it already has
            rather than adding to it, so the number follows the platform
            instead of drifting.
          </li>
        </ul>
      </section>
