...
/**
 * Settings → Privacy & data (coordinator / admin self).
 *
 * Tier-C #116/#117. Same affordances as the couple-side page but
 * routed through the platform endpoints (auth + scope=self).
 */
...
export default function CoordinatorPrivacyPage() {
  ...
  async function handleExport() {
    ...
    setMessage(null)
    try {
      const res = await fetch('/api/agent/me/export', { method: 'POST' })
      if (!res.ok) {
  ...
  async function handleErase() {
    ...
    try {
      const res = await fetch('/api/agent/me/erase', { method: 'POST' })
    ...
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-sage-600">
          Download your platform profile or request your account be deleted.
          Drafts and emails you authored as part of your role belong to the
          venue and are retained as a business record.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="bg-white border border-border rounded-xl p-5">
          ...
          <p className="text-sm text-sage-600 mb-4">
            A JSON file with your platform profile. One download per 30 days.
          </p>
          <button
        ...
        <div className="bg-white border border-border rounded-xl p-5">
          ...
          <p className="text-sm text-sage-600 mb-4">
            Sends an erasure request to your org admin (or platform admin for
            super admins). Reviewed within 45 days.
          </p>
          {confirmDelete ? (
