...
/**
 * Phase E review queue. Lists every open candidate_match for the
 * venue and lets the operator confirm, reject, or defer each one.
 *
 * Anchor: IDENTITY-FIRST-ARCHITECTURE.md §4 + §5. The structured
 * matcher auto-promotes tier=high; medium and low land here for a
 * human decision. The LLM judge fills in the band where the matcher
 * is unsure but stops short of acting. Operator's verdict closes the
 * loop and feeds the calibration / weight-tuning surface in §7.
 *
 * Layout
 * ------
 *  - Top filter row (confidence tier + channel)
 *  - List of candidates, each showing:
 *      - Tier badge + matcher reason (often includes LLM judge note)
 *      - Primary side (the established couple)
 *      - Secondary side (the fragment, touchpoint, or other couple)
 *      - Confirm / Reject / Defer buttons
 *  - Inline action result feedback
 *
 * What's deliberately out of scope here
 * -------------------------------------
 *  - Bulk action UI. Phase E ships per-row. Bulk lands once we know
 *    operator click-through patterns.
 *  - Side-by-side journey overlay. The detail comes via the per-couple
 *    page already; this is the queue.
 */
...
export default function IdentityReviewPage() {
  ...
  const loadRows = useCallback(async () => {
    ...
    // Hydrate each referenced id into a snippet so the queue card
    // can show primary contact name / email / channel without one
  ...
  return (
    <div className="mx-auto max-w-5xl p-8">
      <div className="mb-6">
        <h1 className="font-serif text-3xl text-stone-900">Identity review</h1>
