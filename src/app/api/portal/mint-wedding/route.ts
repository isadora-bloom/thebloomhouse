...
import { provisionCouplePortal } from '@/lib/services/portal/provision'
import {
  getPlatformAuth,
...
export async function POST(request: Request) {
  try {
    ...
    // event_code is no longer generated by the caller. The browser used to
    // mint one and post it here, which made four copies of the format live
    // in the codebase and put a write the browser could not verify in the
    // middle of a booking. A caller may still pass one (imports carrying a
    // code from a CRM), and it is honoured; otherwise provisionCouplePortal
    // mints it below through the single generator.
    ...
    if (
      updateErr &&
      body.eventCode &&
      (updateErr.message?.includes('unique') || updateErr.message?.includes('duplicate'))
    ) {
      // The supplied code collided with another wedding's. Drop it and let
    ...
    // Make the wedding portal-ready: mint the event code if it has none
    // and lay down the wedding_details shell. Best effort by design, so a
    // hiccup here never fails the booking.
    if (!finalEventCode) {
      const provisioned = await provisionCouplePortal(supabase, minted.weddingId)
      finalEventCode = provisioned.event_code
    }
