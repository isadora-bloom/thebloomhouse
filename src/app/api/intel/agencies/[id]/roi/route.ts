...
import { computeAgencyROI } from '@/lib/services/intel/marketing-agencies'
...
/**
 * GET /api/intel/agencies/[id]/roi
 *
 * Compute "is this agency paying off?" answer for the caller's scope.
 *
 * Query params:
 *   ?venue_id=UUID    — single venue (defaults to auth.venueId)
 *   ?org_id=UUID      — every venue in the org
 *   ?window=DAYS      — default 90, clamped 1..3650
 *
 * Returns AgencyROISummary.
 */
export async function GET(request: NextRequest, ctx: RouteContext) {
  ...
  try {
    if (orgIdParam) {
      // Only org_admin / super_admin can fan out by org. Coordinators
      // get their venue back.
    ...
    const summary = await computeAgencyROI({
      agencyId: id,
      venueIds,
      windowDays,
    })
