/**
 * Wave 16 — bulk inquiry-intent reclassification endpoint.
 *
 * Anchor docs:
 ...
 *   - Authorization: Bearer ${CRON_SECRET} → ops path, venueId
 *     REQUIRED in body.
...
import { enqueueIntentClassification } from '@/lib/services/attribution-roles/intent-enqueue'
import { reclassifyVenueIntent } from '@/lib/services/attribution-roles/intent-reclassify-venue'
...
async function resolveAuth(
  ...
  if (auth.isDemo) return forbidden('demo cannot run bulk inquiry-intent reclassify')
...
export async function POST(req: NextRequest) {
  ...
  if (mode === 'sync') {
    const out = await reclassifyVenueIntent({
      venueId,
      limit,
      offset,
      force,
  ...
  if (pageErr) {
    return NextResponse.json(
      { ok: false, error: `page fetch failed: ${pageErr.message}` },
      { status: 500 },
    )
  }
  const events = (rows ?? []) as PickRow[]
