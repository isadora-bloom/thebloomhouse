 ...
 */
type SweepFragmentsForCoupleFn = (
  supabase: SupabaseClient,
  venueId: string,
  coupleId: string,
) => Promise<{ fragmentsPromoted: number }>

async function loadHandlePromotionFn(): Promise<SweepFragmentsForCoupleFn | null> {
  try {
    // Dynamic import: W22's module may not exist yet in this worktree.
    // A missing module throws MODULE_NOT_FOUND, which the catch below
    // turns into a clean skip rather than a build/runtime failure.
    const mod = (await import('./identity-cascade')) as {
      sweepFragmentsForCouple?: SweepFragmentsForCoupleFn
    }
    return mod.sweepFragmentsForCouple ?? null
  } catch {
    return null
  }
}
...
export async function sweepHandlePromotionForVenue(
  ...
  const sweepFragmentsForCouple = await loadHandlePromotionFn()

  if (!sweepFragmentsForCouple) {
    logEvent({
      level: 'info',
      msg: 'fragment_sweep.handle_promotion.not_wired',
      venueId: state.venueId,
      correlationId: state.runId,
      data: {
        note:
          'W22 sweepFragmentsForCouple not found (identity-cascade.ts). ' +
          'Wire this call site up once W22 lands its export.',
      },
    })
    return { couplesWithHandles: 0, fragmentsPromoted: 0, skipped: true, reason: 'not_wired' }
  }
  ...
  const { data } = await state.supabase
    ...
    .not('handles', 'is', null)
    .limit(5000)
  const couples = ((data ?? []) as Array<{ id: string; handles: unknown }>).filter(
    (c) => Array.isArray(c.handles) && c.handles.length > 0,
  )

  let fragmentsPromoted = 0
  for (const c of couples) {
    try {
      const r = await sweepFragmentsForCouple(state.supabase, state.venueId, c.id)
      fragmentsPromoted += r.fragmentsPromoted
    } catch (err) {
      logEvent({
        level: 'warn',
        msg: 'fragment_sweep.handle_promotion.couple_failed',
        venueId: state.venueId,
        correlationId: state.runId,
        data: { couple_id: c.id, error: err instanceof Error ? err.message : String(err) },
      })
    }
  }

  return { couplesWithHandles: couples.length, fragmentsPromoted, skipped: false }
}
