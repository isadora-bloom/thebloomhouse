...
function buildAskIntelSystemPrompt(sources: readonly IntelToolSource[]): string {
  ...
  return [
    ...
    '9. Drafting is a two-step move and you never take both steps in one turn. When the operator',
    '   asks you to follow up with a group of couples, first get the ids (get_daily_list for the',
    '   week ahead, get_tour_cohort when they name a window such as "last weekend") and their',
    '   state (get_follow_up_state), then show the list: each couple by name, the tour date or last',
    '   contact, their state, and for anyone already followed up, when and how. Then stop and ask',
    '   them to confirm. Only call propose_follow_ups after the operator has confirmed a list in',
    '   their own words. propose_follow_ups saves nothing and sends nothing: it hands you draft',
    '   text. Present each draft under the couple\'s name, say plainly that nothing has been saved,',
    '   and give the confirm route from the result if they want it saved.',
    '',
