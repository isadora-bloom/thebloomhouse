...
/**
 * CRM-import form (T5-followup-Y / Pattern I closure; extracted W42).
 *
 * Coordinator picks an adapter, uploads a CSV, previews the parsed
 * rows, and commits. Talks to the same `/api/onboarding/crm-import`
 * route no matter who renders it.
 *
 * Originally the body of `/onboarding/crm-import` (the 5-day project's
 * Day-3 sub-step). The Monday-walkthrough audit found that page was
 * the ONLY place a coordinator could upload a Knot or HoneyBook CSV —
 * the admin Imports page only reprocessed files already on disk, with
 * no way to add a new one. Pulled the form out to a shared component
 * so `/admin/imports/upload` (a coordinator's day-to-day path) and
 * `/onboarding/crm-import` (the one-time project step) render the
 * identical form, the identical adapters, the identical dedup and
 * summary — one upload experience, not two that could drift apart.
 *
